...
class MyBrowser implements Disposable {
  ...
  private async getPage (): Promise<
    Result<
      Page,
      MyError<MyErrorCode> | BrowserError<BrowserErrorCode.MISSING_PAGE>
    >
  > {
    return (await this.getBrowser()).andThenAsync(browser =>
      MyError.try<
        Page,
        MyErrorCode | BrowserErrorCode.MISSING_PAGE,
        MyError<MyErrorCode> | BrowserError<BrowserErrorCode.MISSING_PAGE>
      >(async () => {
  ...
  async login (
    ...
    Result<
      ...
      | BrowserError<
          ...
          | BrowserErrorCode.MISSING_PAGE
        >
      | LoginError<LoginErrorCode.ALREADY_LOGIN>
    >
  > {
    if (this.isLogin)
      return Err(LoginError.new(LoginErrorCode.ALREADY_LOGIN, 'already login'))
    return (await this.browse(url)).andThenAsync(async _ =>
      (await this.getPage()).andThenAsync(async page =>
        MyError.try<
          this,
          number,
          | MyError<MyErrorCode>
  ...
  > {
    return (await this.getPage()).andThenAsync(page =>
      MyError.try(async () => Ok(await page.title()))
    )
  }
